...
serve(async (req) => {
  ...
  try {
    ...
    if (!openaiApiKey) {
      // OpenAI key yoksa basit Türkçe hukuk asistanı yanıtı ver
      const fallbackResponse = generateFallbackResponse(message);
      return new Response(
        JSON.stringify({ data: { message: fallbackResponse } }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    const messages = [
      {
        role: 'system',
        content: 'Sen Yargi AI, Turkiye hukuk sistemi konusunda uzman bir yapay zeka asistani sin. Turk hukuk mevzuati, yargi kararlari, emsal davalar ve hukuki prosedurer hakkinda kapsamli bilgi saglayabilirsin. Yanitlarini Turkce ver, acik ve anlasilir ol. Hukuki tavsiye vermek yerine bilgi sagla ve kullanicayi profesyonel hukuki danismanlik almaya yonlendir.'
      },
      ...conversationHistory || [],
    ...
    const aiMessage = openaiData.choices[0]?.message?.content || 'Uzgunum, su anda yanit uretemiyorum. Lutfen daha sonra tekrar deneyin.';

    return new Response(
      JSON.stringify({ data: { message: aiMessage } }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
    ...
    // Hata durumunda da basit yanıt ver
    const fallbackMessage = generateFallbackResponse('Hata olustu. Lutfen sorunuzu farkli sekilde ifade edin.');
    
    return new Response(
      JSON.stringify({ data: { message: fallbackMessage } }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
  }
});
...
function generateFallbackResponse(userMessage: string): string {
  ...
  if (lowerMessage.includes('anayasa') || lowerMessage.includes('anayasa mahkemesi')) {
    return 'Anayasa Mahkemesi, Turkiye Cumhuriyeti Anayasasi nin yuksek yorumlayicisi ve koruyucusudur. Anayasa degisikliklerini denetler, bireysel basvuru yolunu saglar ve temel hak ve ozgurluklerle ilgili en son inceleme mercii olarak gorev yapar. Hangi konuda anayasa ile ilgili bilgi istiyorsunuz?';
  }
  
  if (lowerMessage.includes('danistay') || lowerMessage.includes('idare')) {
    return 'Danistay, idari yarginin en ust mahkemesidir ve idari islemlerin hukuka uygunlugunu denetler. Idari islem iptali davalari, idari yargi alanina giren uyusmazliklar ve kamu kurumlariyla olan hukuki ihtilaflar Danistayin gorev alanina girer. Spesifik bir idari hukuk konusunda yardima ihtiyaciniz var mi?';
  }
  
  if (lowerMessage.includes('yargitay') || lowerMessage.includes('ceza') || lowerMessage.includes('hukuk')) {
    return 'Yargitay, hukuk ve ceza yargi alanlarinin en ust mahkemesidir. Hukuk Dairesi ve Ceza Dairesi olarak hizmet verir ve hukuk uygulamasinda yeknesakligi saglar. Hukuki bir sorunuz varsa hangi alanda yardima ihtiyaciniz var?';
  }
  
  if (lowerMessage.includes('sayistay') || lowerMessage.includes('denetim')) {
    return 'Sayistay, devletin gelir, gider ve mallarinin hukuki denetimini yapan anayasal bir kurumdur. Mali islemlerin denetimi, hesap ve belgelerin incelenmesi, tespit edilen eksikliklerin giderilmesi icin uyarilarda bulunma gibi gorevleri vardir. Hangi Sayistay denetimi konusunda bilgi istiyorsunuz?';
  }
  
  if (lowerMessage.includes('kik') || lowerMessage.includes('ihale') || lowerMessage.includes('kamu ihale')) {
    return 'Kamu Ihale Kurulu (KIK), kamu ihale mevzuati nin uygulanmasini izleyen ve ihale sureclerinde taraflarin haklarini koruyan bagimsiz idari otoritidir. Kamu Ihale Kanunu kapsamindaki tum ihaleler ve ihale surecleri KIKin gorev alanina girer. Ihale hukuku konusunda spesifik bir sorunuz var mi?';
  }
  
  if (lowerMessage.includes('kvkk') || lowerMessage.includes('kisisel veri') || lowerMessage.includes('veri koruma')) {
    return 'Kisisel Verilerin Korunmasi Kanunu (KVKK), kisisel verilerin islenmesi, korunmasi ve aktarilmasi ile ilgili duzenlemeler icerir. Kisisel Verileri Koruma Kurumu bu alanda denetim yapan ve idari para cezalari uygulayan kurumdur. KVKK ile ilgili hangi konuda bilgi istiyorsunuz?';
  }
  
  if (lowerMessage.includes('bosanma') || lowerMessage.includes('velayet') || lowerMessage.includes('nafaka')) {
    return 'Aile hukuku konularinda (bosanma, velayet, nafaka, mal paylasimi) aile mahkemelerinde yargilama yapilir. Aile hukuku davalarinda cocuklarin menfaati her zaman onceliktir. Aile hukuku konusunda hangi spesifik konuda yardima ihtiyaciniz var?';
  }
  
  if (lowerMessage.includes('tazminat') || lowerMessage.includes('zarar') || lowerMessage.includes('kaza')) {
    return 'Tazminat hukuku, haksiz fiil nedeniyle olusan zararlarin karsilanmasi icin basvurulan hukuki yoldur. Is kazasi, trafik kazasi, mal varligina zarar gibi durumlarda tazminat talep edilebilir. Tazminat hukuku konusunda hangi spesifik alanda bilgi istiyorsunuz?';
  }

  return 'Bu konuda daha spesifik bilgi saglayabilmem icin lutfen sorunuzu daha ayrintili olarak ifade edin. Turk hukuk sistemi hakkinda herhangi bir konuda size yardimci olmaya calisacagim. Ancak aldiginiz hukuki bilgilerin profesyonel danismanlikla desteklenmesini oneririm.';
}
>>>>>>> 99cfd8a (feat: Add Supabase Edge Functions and updated backend logic)
