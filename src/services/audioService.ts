...
export class AudioService {
  private settings: AudioSettings = {
    useApiAudio: true, // OpenAI TTS'i öncelikli kullan
    fallbackToTts: true, // Fallback olarak native TTS
    voiceSpeed: 0.9,
    voicePitch: 1.0
  };
  ...
  private useOpenAI = true; // Varsayılan olarak OpenAI TTS dene
  ...
  async playAudio(script: AudioScript): Promise<void> {
    ...
    try {
      // Önce OpenAI TTS'i dene
      if (this.settings.useApiAudio && this.useOpenAI) {
        try {
          console.log('OpenAI TTS deneniyor...');
          const fullText = `${script.title}. ${script.text}`;
          await openaiTTSService.playAudio(fullText);
          console.log('OpenAI TTS başarılı');
          return;
        } catch (openaiError) {
          console.warn('OpenAI TTS başarısız, native TTS\'e geçiliyor:', openaiError);
          this.useOpenAI = false; // Bu session için OpenAI TTS'i devre dışı bırak
        }
      }

      // ElevenLabs başarısız veya devre dışı, native TTS kullan
      if (this.settings.fallbackToTts) {
        ...
        throw new Error('Ses çalma seçenekleri tükendi');
      }
    } catch (error) {
      console.error('Ses çalma hatası:', error);
      
      // Son çare: basitleştirilmiş TTS
      if (this.settings.fallbackToTts) {
        try {
  ...
  stopAudio(): void {
    ...
    // OpenAI TTS audio'yu durdur
    try {
      openaiTTSService.stopAudio();
    } catch (error) {
