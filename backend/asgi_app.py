...
if auth_enabled:
    try:
        # Import FastMCP Auth Providers only if enabled
        ...
        logger.error(f"Authentication enabled but FastMCP auth providers not available: {e}")
        logger.warning("Falling back to disabled authentication")
        auth_enabled = False
...
class SearchRequest(BaseModel):
    ...
    esasYil: Optional[str] = None
    kararYil: Optional[str] = None

async def call_mcp_tool(tool_name: str, arguments: Dict[str, Any]):
    """Call an MCP tool with given arguments, handling Pydantic Field defaults."""
    try:
        import inspect
        ...
        try:
            ...
            # Fallback for older pydantic if necessary
            PydanticUndefined = getattr(FieldInfo, "__pydantic_undefined__", None)
        
        # Access internal tool manager of FastMCP
        tool = mcp_server._tool_manager._tools.get(tool_name)
        if not tool:
            raise HTTPException(status_code=404, detail=f"Tool '{tool_name}' not found")
        
        # Inspect signature to fill defaults
        sig = inspect.signature(tool.fn)
        filled_args = {}
        
        for name, param in sig.parameters.items():
            if name in arguments and arguments[name] is not None:
                filled_args[name] = arguments[name]
            elif isinstance(param.default, FieldInfo):
                # If it's a FieldInfo, use its default value or factory
                if param.default.default is not PydanticUndefined:
                    filled_args[name] = param.default.default
                elif param.default.default_factory is not None:
                    filled_args[name] = param.default.default_factory()
                else:
                    filled_args[name] = None
            else:
                # Regular Python default or no default
                if param.default is not inspect.Parameter.empty:
                    filled_args[name] = param.default
        
        # Execute tool function with properly filled arguments
        result = await tool.fn(**filled_args)
        return result
...
async def search_yargitay(request: SearchRequest):
    args = {
        ...
        "birimYrgKurulDaire": request.daire or "ALL"
    }
    if request.esasYil:
        args["esasYil"] = request.esasYil
    if request.kararYil:
        args["kararYil"] = request.kararYil
        
    return await call_mcp_tool("search_yargitay_detailed", args)

@app.post("/api/danistay/search-keyword")
async def search_danistay_keyword(request: SearchRequest):
    # Map single keyword to AND list as expected by Danistay tool
    args = {
        "andKelimeler": [request.keyword],
        "pageNumber": request.page
    }
